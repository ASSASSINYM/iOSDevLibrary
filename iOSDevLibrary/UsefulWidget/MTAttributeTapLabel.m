...
#import "MTAttributeTapLabel.h"

#pragma mark - MTAttributeModel

@implementation MTAttributeModel
@end

@interface MTAttributeTapLabel()
...
@implementation MTAttributeTapLabel

- (void)setText:(NSString *)text
     attributes:(NSDictionary *)attr
 tapStringArray:(NSArray <MTAttributeModel *>*)stringArray
{
    ...
    [stringArray enumerateObjectsUsingBlock:^(MTAttributeModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    for (CFIndex i = 0; i < numberOfLines; i++) {
        ...
        if (CGRectContainsPoint(rect, p)) {
            ...
            for (int j = 0; j < link_count; j++) {
                MTAttributeModel *model = self.targetStringArray[j];
